...
def load_triviaqa(
    *,
    ...
    """Filter TriviaQA rows to wiki-resolvable entities, return BfclCase list.

    When `verifier_fn` is provided, each candidate row goes through the LLM
    verifier — critical for TriviaQA where broad-domain trivia questions
    frequently hit our slug names as unrelated entities (Jean-Claude Killy,
    Gemini zodiac, etc.).
    """
    ...
    for row in dataset_iter:
        ...
        if n_max is not None and len(out) >= n_max:
            break
    log.info(
        "triviaqa: examined %d rows, kept %d (verifier dropped %d)",
        examined, len(out), dropped_by_verifier,
    )
    return out
